...
describe("Marketplace", function () {
    ...
    describe("putForSale - 25 points", function () {
        ...
        it("Should fail if the transaction sender is not the owner of the asset - 4 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);
            await expect(contract.connect(account2).putForSale(MINMUM_BID_AUCTION_EXISTS_FIXTURE, 0)).to.be.reverted;
        });
        it("Should increase auctionNumber by 1 after successful auction - 4 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);
            let contractAuctionNumber = await contract.auctionNumber();
            expect(contractAuctionNumber).to.equal(1);
        });
        it("Should set idToAuction mapping for auctionNumber - 3 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);
        ...
        it("Should set ownerToAuctionId mapping for transaction sender - 3 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);

            let auctionNumber = await contract.ownerToAuctionId(account);
            expect(auctionNumber).to.equal(1);
        });
        it("Should set auctionToObject mapping for auctionNumber - 2 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);
    ...
    describe("bid - 15 points", function () {
        it("Should bid for the auction correlated with the assetId - 15 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionExistsFixture);
    ...
    describe("settleAuction - 10 points", function () {
        it("Should end the auction if called successfully - 5 points", async function () {
        ...
        it("Should transfer the asset to the max bidder if called successfully - 5 points", async function () {
            const { contract, account, account2, account3 } = await loadFixture(auctionHasBidsFixture);
